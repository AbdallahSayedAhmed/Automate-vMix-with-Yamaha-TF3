...
export const VMIX_FN_INFO = {
  ...
  OverlayInput1: "Shows/toggles the selected input on Overlay channel 1.",
  OverlayInput2: "Shows/toggles the selected input on Overlay channel 2.",
  OverlayInput3: "Shows/toggles the selected input on Overlay channel 3.",
  OverlayInput4: "Shows/toggles the selected input on Overlay channel 4.",
  OverlayInput1In: "Turns the selected input ON in Overlay channel 1.",
  OverlayInput2In: "Turns the selected input ON in Overlay channel 2.",
  OverlayInput3In: "Turns the selected input ON in Overlay channel 3.",
  OverlayInput4In: "Turns the selected input ON in Overlay channel 4.",
  OverlayInput1Out: "Turns Overlay channel 1 OFF for the selected input.",
  OverlayInput2Out: "Turns Overlay channel 2 OFF for the selected input.",
  OverlayInput3Out: "Turns Overlay channel 3 OFF for the selected input.",
  OverlayInput4Out: "Turns Overlay channel 4 OFF for the selected input.",
  OverlayInputAllOff: "Turns all vMix overlay channels off.",
  PreviewInput: "Sends an input to Preview.",
  Cut: "Cuts a selected input to Program.",
  Fade: "Fades a selected input to Program.",
};
...
export const VMIX_FN_LABELS = {
  ...
  OverlayInput1: "Overlay 1 — Toggle Input",
  OverlayInput2: "Overlay 2 — Toggle Input",
  OverlayInput3: "Overlay 3 — Toggle Input",
  OverlayInput4: "Overlay 4 — Toggle Input",
  OverlayInput1In: "Overlay 1 — Input ON",
  OverlayInput2In: "Overlay 2 — Input ON",
  OverlayInput3In: "Overlay 3 — Input ON",
  OverlayInput4In: "Overlay 4 — Input ON",
  OverlayInput1Out: "Overlay 1 — Input OFF",
  OverlayInput2Out: "Overlay 2 — Input OFF",
  OverlayInput3Out: "Overlay 3 — Input OFF",
  OverlayInput4Out: "Overlay 4 — Input OFF",
  OverlayInputAllOff: "All Overlays — OFF",
  PreviewInput: "Preview — Set Input",
  Cut: "Program — Cut Input",
  Fade: "Program — Fade Input",
};

export function vmixFnNeedsInput(fn) {
  if (fn === "SetVolume") return true;
  if (fn?.startsWith("OverlayInput") && fn !== "OverlayInputAllOff")
    return true;
  return ["PreviewInput", "Cut", "Fade"].includes(fn);
}

export function vmixFnNeedsValue(fn) {
  if (!fn) return true;
  if (fn === "OverlayInputAllOff") return false;
  if (fn?.startsWith("OverlayInput")) return false;
  if (["PreviewInput", "Cut", "Fade"].includes(fn)) return false;
  return true;
}
...
export function formatListenDetail(rule, vmixInputs = []) {
  ...
  let inputLabel = "Any vMix Input";
  let isWarning = false;
  if (rule.vmix_input_key) {
    const liveInput = vmixInputs.find((i) => i.key === rule.vmix_input_key);
    if (liveInput) {
      inputLabel = `Input ${liveInput.number}: ${liveInput.title}`;
    } else {
      inputLabel = `Input Missing (was ${rule.vmix_input_name})`;
      isWarning = true;
    }
  } else if (rule.vmix_input_name) {
    const liveInput = vmixInputs.find((i) => i.title === rule.vmix_input_name);
    if (liveInput) {
      inputLabel = `Input ${liveInput.number}: ${liveInput.title}`;
    } else {
      inputLabel = `Input Missing (was ${rule.vmix_input_name})`;
  ...
  } else if (rule.vmix_input_number) {
    const liveInput = vmixInputs.find((i) => i.number === rule.vmix_input_number);
    if (liveInput) {
      inputLabel = `Input ${rule.vmix_input_number}: ${liveInput.title}`;
    } else {
      inputLabel = `Input ${rule.vmix_input_number} (Missing)`;
      isWarning = true;
    }
  }
  const eventLabel =
  ...
  return {
    badge: "VMIX EVENT",
    badgeColor: isWarning ? "#FF4B4B" : "#20D9FF",
    primary: eventLabel,
...
export function formatCommandDetail(rule) {
  ...
  const actionList = Array.isArray(rule.actions)
    ...
    : typeof rule.actions === "string"
      ...
      : [];
  if (
    !(rule.listen_source === "yamaha" && rule.is_multi_duck) &&
    (rule.is_multi_action || actionList.length > 0)
  ) {
    const acts = actionList.length ? actionList : [rule];
