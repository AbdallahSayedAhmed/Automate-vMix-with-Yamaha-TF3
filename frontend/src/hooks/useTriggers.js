...
export function useTriggers() {
  ...
  useEffect(() => {
    const loadTimer = setTimeout(fetchTriggers, 0);
  ...
  const addTrigger = async (triggerData) => {
    ...
    setTriggers((prev) =>
      [...prev, newTrigger].sort((a, b) => a.sort_order - b.sort_order),
    );
    return newTrigger;
  };
  ...
  const toggleTrigger = async (id) => {
    ...
    if (prev) {
      setTriggers((ts) =>
        ts.map((t) => (t.id === id ? { ...t, is_active: !t.is_active } : t)),
      );
    }
    try {
  ...
  const bulkToggle = async (ids, is_active) => {
    setTriggers((prev) =>
      prev.map((t) => (ids.includes(t.id) ? { ...t, is_active } : t)),
    );
    try {
  ...
  const bulkCreate = async (rules) => {
    ...
    setTriggers((prev) =>
      [...prev, ...newRules].sort((a, b) => a.sort_order - b.sort_order),
    );
    return newRules;
  };
