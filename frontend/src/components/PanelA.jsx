...
function buildRestorePayload(rule) {
  ...
  return {
    ...
    is_multi_action: rule.is_multi_action ?? true,
...
function normalizeAction(action, fallback = {}, sortOrder = 0) {
  return {
    action_target: action?.action_target || fallback.action_target || "yamaha",
    yamaha_command:
      action?.yamaha_command || fallback.yamaha_command || "InCh/Fader/Level",
    yamaha_channel: action?.yamaha_channel ?? fallback.yamaha_channel ?? 1,
    yamaha_mix: action?.yamaha_mix ?? fallback.yamaha_mix ?? 0,
    vmix_function: action?.vmix_function || fallback.vmix_function || null,
    vmix_target_input:
      action?.vmix_target_input ?? fallback.vmix_target_input ?? null,
    parameter_value: String(
      action?.parameter_value ?? fallback.parameter_value ?? "0",
    ),
    delay_ms: action?.delay_ms ?? fallback.delay_ms ?? 0,
    sort_order: action?.sort_order ?? sortOrder,
  };
}
...
function normalizeMember(member, index) {
  ...
  const actions =
    Array.isArray(member.actions) && member.actions.length
      ? member.actions.map((action, actionIndex) =>
          normalizeAction(action, fallback, actionIndex),
        )
      : [normalizeAction(fallback, fallback, 0)];
  const first = actions[0];

  return {
    ...member,
    monitor_channel: member.monitor_channel ?? index + 1,
    threshold: member.threshold ?? -4000,
    release_threshold: member.release_threshold ?? -5000,
    actions,
    action_target: first.action_target,
    yamaha_command: first.yamaha_command,
    yamaha_channel: first.yamaha_channel,
    yamaha_mix: first.yamaha_mix,
    vmix_function: first.vmix_function,
    vmix_target_input: first.vmix_target_input,
...
function normalizeMultiDuckPayload(payload) {
  const isYamahaMultiDuck =
    payload.listen_source === "yamaha" && payload.is_multi_duck;

  if (!isYamahaMultiDuck) {
    const actions =
      Array.isArray(payload.actions) && payload.actions.length
        ? payload.actions.map((action, index) =>
            normalizeAction(action, payload, index),
          )
        : [normalizeAction(payload, payload, 0)];
    const first = actions[0];
    return {
      ...payload,
      is_multi_duck: false,
  ...
  const members = Array.isArray(payload.duck_members)
  ...
  return {
    ...
    duck_members: members.map(normalizeMember),
  };
}
...
export const PanelA = React.memo(function PanelA({
  ...
  useEffect(() => {
    ...
    const fetchInputs = async ({ manual = false } = {}) => {
      try {
        const inputs = await api.getVmixInputs();
        if (!alive) return;
        setVmixInputs(inputs);
        if (manual) {
          toast.success(`vMix Inputs Refreshed (${inputs.length})`, {
            duration: 1800,
          });
        }
      } catch (error) {
        if (!alive) return;
        console.error("Failed to refresh vMix inputs", error);
        if (manual) {
          toast.error(error?.message || "Failed to refresh vMix inputs", {
            duration: 3500,
          });
        }
      }
    ...
    const onRefreshInputs = (event) => {
      if (debounceTimer) clearTimeout(debounceTimer);
      const manual = Boolean(event?.detail?.manual);
      debounceTimer = setTimeout(() => fetchInputs({ manual }), 200);
    };

    fetchInputs();

    window.addEventListener("refresh-vmix-inputs", onRefreshInputs);
    return () => {
      alive = false;
      if (debounceTimer) clearTimeout(debounceTimer);
      window.removeEventListener("refresh-vmix-inputs", onRefreshInputs);
    };
  }, []);
  ...
  const keyboardRef = useLatestRef({
    blocked:
      confirmModal.isOpen || showGroupModal || shortcutsOpen || undoConfirmOpen || commandPaletteOpen,
    editorOpen,
    commandPaletteOpen,
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      } else if (ctx.prefs.matchShortcut(e, "commandPalette") && !isInput) {
  ...
  return (
    <>
      ...
      <CommandPalette
        ...
        triggers={triggers}
        presets={[]} // Add presets if available, or empty array
        onToggleRule={toggleTrigger}
        onEditRule={handleEditClick}
        onDeleteRule={handleDeleteRule}
        onNewRule={handleCreateNew}
        onRunPreset={() => {}}
        onSelectAll={handleSelectAll}
        onToggleAllRules={() => handleBulkToggle(!allSelectedActive)}
...
function actionStateKey(ruleId, member, memberIndex, actionIndex = null) {
  if (memberIndex != null) {
    if (actionIndex != null) {
      return `${ruleId}:member_index:${memberIndex}:action:${actionIndex}`;
    }
    return `${ruleId}:member_index:${memberIndex}`;
  }
  const channel = member?.monitor_channel ?? "unknown";
  if (actionIndex != null) {
    return `${ruleId}:monitor_channel:${channel}:action:${actionIndex}`;
  }
  return `${ruleId}:monitor_channel:${channel}`;
}

function formatVmixActionDetail(action) {
  const fn = action.vmix_function || "SetVolume";
  const target = action.vmix_target_input || action.parameter_value;
  if (vmixFnNeedsInput(fn) && fn !== "SetVolume") {
    return `Input ${target || "?"}`;
  }
  if (!vmixFnNeedsValue(fn)) return "No value";
  return `${action.vmix_target_input ? `Input ${action.vmix_target_input} → ` : ""}${action.parameter_value}`;
}
...
const RuleRow = React.memo(function RuleRow({
  ...
  useEffect(() => {
    ...
    const stopFlash = setTimeout(() => setFlash(false), 300);
    return () => {
  ...
  const rowStyle = flash
    ...
    : {
        ...
        borderBottom: "1px solid rgba(255,255,255,0.04)",
        opacity: trigger.is_active ? 1 : 0.35,
        filter: trigger.is_active ? "none" : "brightness(0.55) saturate(0.65)",
      };
  ...
  const getMemberActionState = (member, memberIndex, actionIndex = null) =>
    actionStates[
      actionStateKey(trigger.id, member, memberIndex, actionIndex)
    ] ||
    (actionIndex != null
      ? actionStates[
          `${trigger.id}:monitor_channel:${member.monitor_channel}:action:${actionIndex}`
        ]
      : null) ||
    actionStates[actionStateKey(trigger.id, member, memberIndex)] ||
    actionStates[`${trigger.id}:monitor_channel:${member.monitor_channel}`] ||
  ...
  const actionsList = (() => {
    if (trigger.listen_source === "yamaha" && trigger.is_multi_duck) return [];
    if (Array.isArray(trigger.actions) && trigger.actions.length) {
      return trigger.actions;
    }
    if (typeof trigger.actions === "string") {
      try {
        const parsed = JSON.parse(trigger.actions);
        if (Array.isArray(parsed) && parsed.length) return parsed;
      } catch {
        return trigger.is_multi_action ? [trigger] : [];
      }
    }
    return trigger.is_multi_action ? [trigger] : [];
  })();
  ...
  return (
    ...
    >
      <td
        className="rule-cell rule-cell--select"
        style={{ padding: "8px", ...leftBdr, background: cellBg }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
          {listeners && (
            <div
      ...
      <td
        className="rule-cell rule-cell--name"
        style={{ padding: "8px 12px", background: cellBg }}
      >
        <div style={{ display: "flex", alignItems: "center", gap: "7px" }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            >
              ...
              <span
                title={
                  trigger.is_active ? "Rule is enabled" : "Rule is disabled"
                }
                style={{
                  flexShrink: 0,
                  borderRadius: "999px",
                  ...
                  letterSpacing: "0.08em",
                  textTransform: "uppercase",
                  color: trigger.is_active ? "#39E58C" : "#FF5C7A",
                  background: trigger.is_active
                    ? "rgba(57,229,140,0.1)"
                    : "rgba(255,92,122,0.1)",
                  border: `1px solid ${trigger.is_active ? "rgba(57,229,140,0.28)" : "rgba(255,92,122,0.28)"}`,
                }}
              >
            ...
            {!trigger.is_multi_duck && actionsList.length > 0 && (
              <button
                type="button"
                onClick={(e) => {
                  e.stopPropagation();
      ...
      <td
        className="rule-cell rule-cell--listen"
        style={{
          padding: "8px 14px",
      ...
      >
        ...
        <div style={{ fontSize: "11px", color: listen.isWarning ? "#FF4B4B" : "#8B93A8", marginTop: "2px" }}>
          {listen.secondary}
        </div>
          ...
          actionsList.length > 0 && (
            ...
            >
              ...
              <div
                ...
                title={listen.secondary}
              >
                <span style={{ color: listen.isWarning ? "#FF4B4B" : "inherit" }}>
                  {listen.secondary}
                </span>
              </div>
              <div
              ...
              >
                <div
                  style={{
                    height: "100%",
                    width: "100%",
                    background: "linear-gradient(90deg, #20D9FF, #39E58C)",
                  }}
                />
              </div>
            </div>
      ...
      <td
        className="rule-cell rule-cell--command"
        style={{
          padding: "8px 14px",
          background: isGrouped
      ...
      >
        ...
        {trigger.is_multi_duck && multiExpanded && duckMembers.length > 0 && (
          ...
          >
            {duckMembers.map((m, i) => {
              const memberActions = getMemberActions(m);
              return (
                <div
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ color: "#5A6278", fontSize: "9px" }}>
                      {memberActions.length} action
                      {memberActions.length === 1 ? "" : "s"}
                    </span>
                  </div>
                  ...
                  >
                    {memberActions.map((action, actionIndex) => {
                      const state = getMemberActionState(m, i, actionIndex);
                      const status = actionStateStyle(state?.status);
                      const isVmix = action.action_target === "vmix";
                      const cmdLabel = isVmix
                        ? VMIX_FN_LABELS[action.vmix_function] ||
                          action.vmix_function ||
                          "vMix"
                        : YAMAHA_CMD_LABELS[action.yamaha_command] ||
                          action.yamaha_command ||
                          "Yamaha";
                      const detail = isVmix
                        ? formatVmixActionDetail(action)
                        : `Ch ${action.yamaha_channel || "?"}${action.yamaha_mix ? ` Mix ${action.yamaha_mix}` : ""} → ${action.parameter_value}`;

                      return (
                        <div
                          key={`cmd-${m.monitor_channel}-${i}-${actionIndex}`}
                          style={{
                            padding: "5px 6px",
                            borderRadius: "6px",
        ...
        {!trigger.is_multi_duck && multiExpanded && actionsList.length > 0 && (
          <div
            style={{
              marginTop: "8px",
              display: "flex",
      ...
      <td
        className="rule-cell rule-cell--fires"
      ...
      <td
        className="rule-cell rule-cell--last"
        style={{ padding: "8px 8px", background: cellBg }}
      >
        <span
          style={{
            fontSize: "11px",
      ...
      <td
        className="rule-cell rule-cell--actions"
      ...
      <td
        className="rule-cell rule-cell--state"
        style={{ padding: "8px 4px", textAlign: "center", background: cellBg }}
      >
