...
const DEFAULT_ACTION = {
  ...
  vmix_target_input_key: null,
  parameter_value: "0",
  delay_ms: 0,
};

export function MultiActionFields({ form, onChange, vmixInputs = [] }) {
  ...
  const formatActionDetail = (action) => {
    ...
    if (vmixFnNeedsInput(fn) && fn !== "SetVolume") {
      return `${VMIX_FN_LABELS[fn] || fn} → Input ${target || "?"}`;
    }
    if (!vmixFnNeedsValue(fn)) return VMIX_FN_LABELS[fn] || fn;
    return `${VMIX_FN_LABELS[fn] || fn} → ${action.parameter_value}`;
  };
    ...
    <div className="space-y-4">
      <div className="space-y-3">
        {actions.map((action, idx) => {
          ...
          const isSmooth =
            action.action_target === "yamaha" && cmd.endsWith("/Smooth");
          const vmixFn = action.vmix_function || "SetVolume";
          const needsVmixInput =
            action.action_target === "vmix" && vmixFnNeedsInput(vmixFn);
          const needsVmixValue =
            action.action_target !== "vmix" || vmixFnNeedsValue(vmixFn);
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex gap-1">
                  ...
                  <button
                    ...
                    style={{
                      color: "#f87171",
                      opacity: actions.length <= 1 ? 0.3 : 1,
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      onChangeAction(idx, "action_target", e.target.value)
                    }
                    style={inputStyle}
                  >
                ...
                {action.action_target === "yamaha" ? (
                  <div>
                    ...
                    <select
                      value={cmd}
                      onChange={(e) =>
                        onChangeAction(idx, "yamaha_command", e.target.value)
                      }
                      style={inputStyle}
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        onChangeAction(idx, "vmix_function", e.target.value)
                      }
                      style={inputStyle}
                    >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          actionsCopy[idx] = { ...actionsCopy[idx], vmix_target_input: num };
                          const inp = vmixInputs.find((i) => i.number === num);
                          actionsCopy[idx].vmix_target_input_key = inp?.key || null;
                      ...
                      >
                        ...
                        {vmixInputs.map((i) => (
                          <option key={i.number} value={i.number} style={{ background: "#151B27" }}>
                            {i.number}: {i.title}
                          </option>
                        ))}
                      </select>
                ...
                {isSmooth ? (
                    ...
                    return (
                      <>
                        <div>
                          <label
                            style={labelStyle}
                            title="Level to fade TO in integer dB (-2000 = -20dB)"
                          >
                            End Level (dB)
                          </label>
                          <input
                        ...
                        <div>
                          <label
                            style={labelStyle}
                            title="Duration of the fade in milliseconds"
                          >
                            Duration (ms)
                          </label>
                          <input
                  ...
                  <div className={needsChannel && needsMix ? "col-span-2" : ""}>
                    <label
                      style={labelStyle}
                      title={
                        action.action_target === "vmix"
                          ? "vMix value, such as volume 0–100."
                          : cmd === "ssrecall_ex"
                            ? "Scene Number to recall"
                            : "For levels: integer dB (0 = Unity). For On/Off: 1 or 0."
                      }
                    >
                      {action.action_target === "vmix"
                        ? "Value"
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        onChangeAction(idx, "parameter_value", e.target.value)
                      }
                      style={{ ...inputStyle, fontFamily: "monospace" }}
                    />
                  </div>
