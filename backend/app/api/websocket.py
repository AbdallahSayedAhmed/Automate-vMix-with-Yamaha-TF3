...
class ConnectionManager:
    ...
    async def broadcast_listen_event(self, payload: Dict[str, Any]):
        await self._broadcast({
            "type": "LISTEN_EVENT",
            "data": payload
        })

    async def broadcast_vmix_inputs_changed(self, payload: Dict[str, Any]):
        await self._broadcast({
            "type": "VMIX_INPUTS_CHANGED",
            "data": payload
        })
...
async def on_vmix_inputs_changed(payload: Dict[str, Any]):
    await ws_manager.broadcast_vmix_inputs_changed(payload)
...
engine.add_vmix_inputs_callback(on_vmix_inputs_changed)
