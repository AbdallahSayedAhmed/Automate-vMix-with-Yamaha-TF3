...
class GroupDuckEngine:
    ...
    async def _process_member_meter(
        ...
        if level >= threshold:
            ...
            if not was_speaking:
                rt["speaking"] = True
                if hasattr(engine, "_broadcast_listen_event"):
                    await engine._broadcast_listen_event(
                        source="yamaha",
                        event_name="YamahaMeter",
                        rule={"id": gdict["id"], "name": gdict["name"]},
                        listen_input=mdict.get("monitor_channel"),
                        listen_label=f"Mic Ch {mdict.get('monitor_channel')}",
                        action_rules=action_rules,
                        extra={
                            "member_id": mdict.get("id"),
